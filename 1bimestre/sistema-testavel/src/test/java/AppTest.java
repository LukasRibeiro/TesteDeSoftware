
/*
 * This Java source file was generated by the Gradle 'init' task.
 */

import javax.swing.JButton;
import javax.swing.JTextField;
import static org.junit.jupiter.api.Assertions.assertEquals;
import org.junit.jupiter.api.Test;

import Model.AcoesBotao;
import model.AnalisadorSequencia;
import Model.Janela;

public class AppTest {

AnalisadorSequencia analyse = new AnalisadorSequencia();

Janela j = new Janela();

@Test
// Primeira sequencia correta
public void sequecia1Correta() throws InterruptedException {
    
    Janela app = new Janela();

    app.getValorEntrada().setText("10,10,10,10");
    
    Thread th = Thread.currentThread();
    
    th.sleep(5000);
    
    app.getBotaoTestar().doClick();
    assertEquals("false", app.getValorSaida().getText());    
    }
}


//     j
//     app.getFieldValor().setText("09,30,40,40");
//     Thread th = Thread.currentThread();
//     th.sleep(5000);
//     app.getBotaoTestar().doClick();
//     assertEquals("false", app.getFieldResultado().getText());
// }
}







/*===============================================MODELO ERRADO=============================================*/    
    
    
    // Janela window = new Janela();

    // window.setVisible(true);

    // JTextField entrada = window.getValorEntrada();
    // JTextField saida = window.getValorSaida();
    // JButton botaoTestar = window.getBotaoTestar();

    // Thread th = Thread.currentThread();

    // th.sleep(5000);

    // entrada.setText("10,20,30,40"); 
    
    // th.sleep(5000);

    // botaoTestar.doClick();
    
    // th.sleep(5000);


// public void Sequencia1 (){
    //     assertEquals(true, analyse.analisar("9,10,11,12"));
    // }

    
    // public void Sequencia2 (){
    //     assertEquals(true, analyse.analisar("9,10,11,12"));
    // }

    
    // public void Sequencia3 (){
    //     assertEquals(true, analyse.analisar("9,10,11,12"));
    // }


    // AcoesBotao act = new AcoesBotao();
  
    // //Primeira sequencia de numeros correta
    // @Test
    // public void PrimeiraSequencia(){
        
    //     JTextField sequencia1 = new JTextField();
    //     sequencia1.setText("10,20,30,40");
    //     assertEquals(true, act.testarCampos(sequencia1));
    // }
    
    // //Segunda sequencia incorreta com 0 na frente
    // @Test
    // public void SegundaSequencia(){
        
    //     JTextField sequencia2 = new JTextField();
    //     sequencia2.setText("01,02,30,40");
    //     assertEquals(true, act.testarCampos(sequencia2));
    // }
    
    // //Terceira sequencia incorreta com numeros de 3 algarismos
    // @Test
    // public void TerceiraSequencia(){
        
    //     JTextField sequencia3 = new JTextField();
    //     sequencia3.setText("100,200,30,40");
    //     assertEquals(true, act.testarCampos(sequencia3));
    // }

    // //Quarta sequencia correta de numeros
    // @Test
    // public void QuartaSequencia(){
        
    //     JTextField sequencia4 = new JTextField();
    //     sequencia4.setText("20,30,40,50");
    //     assertEquals(true, act.testarCampos(sequencia4));
    // }